...
- `sessionMessage/received` — `{ threadId, turnId, senderThreadId, message }` when a peer session delivers a cross-session message to the thread; `turnId` is the turn that received it. Emitted regardless of `thread/start.experimentalRawEvents` (the matching `rawResponseItem/completed` stays opt-in).
