...
#[tokio::test]
async fn only_messages_for_tracked_threads_are_recorded() -> color_eyre::Result<()> {
    let mut app = crate::app::test_support::make_test_app().await;
    let app_server =
        crate::start_embedded_app_server_for_picker(app.chat_widget.config_ref()).await?;
    let primary_thread_id = ThreadId::new();
    app.primary_thread_id = Some(primary_thread_id);
    let mut primary_notification = notification("for this session");
    primary_notification.thread_id = primary_thread_id.to_string();
    ...
    for received in [
        ...
        app.handle_app_server_event(
            ...
            codex_app_server_client::AppServerEvent::ServerNotification(Box::new(
                ServerNotification::SessionMessageReceived(received),
            )),
        )
        .await;
    }
    ...
    assert!(
        app.received_messages
            .viewer_lines(Some(primary_thread_id))
            .iter()
            .any(|line| {
                line.spans
                    .iter()
